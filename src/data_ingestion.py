...
def load_data(data_url: str)->pd.DataFrame:
    try:
        ...
        logger.error('Failed to parse the CSV file from {data_url}')
        raise
    except Exception as e:
        logger.error('Error: An Unexpected error occured while loading the data')
        raise
...
def preprocess_data(df: pd.DataFrame) -> pd.DataFrame:
    try:
        ...
        logger.error('Error: Missing column {e} in the dataframe')
        raise
    except Exception as e:
        logger.error('Error: An unexpected error occured during preprocessing')
        raise
...
def save_data(train_data: pd.DataFrame, test_data:pd.DataFrame, data_path: str)-> None:
    try:
        ...
        logger.error('Error: An unexpected error occured while saving the data')
        raise
